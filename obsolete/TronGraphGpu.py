...
from TestTrxAddress import TrxAddress, get_first_trx_min_common_level
...
class TronGraph:
    def __init__(self, W, gamma=0.1, coef0=0) -> None:
    ...
    def explore_illegal_routes(
        ...
        def get_tainted_weight(address):
            ...
            weights = np.fromiter((self.usdt_graph.nodes[neighbor]["tainted_weight"] for neighbor in neighbors), float,num_neighbors)
            (
                sum_weight,
                avg_weight,
                # std_weight,
            ...
            usdt_weights = np.fromiter((self.usdt_tainted_dict[neighbor] for neighbor in neighbors), float, num_neighbors)
            (
                sum_usdt_weight,
                avg_usdt_weight,
                # std_usdt_weight,
            ...
            if gas_path:
                for path in self.gas_graph_dict.values():
                    if path != gas_path:
                        path = np.array(path)
                        gas_path = np.array(gas_path)
                        level = get_first_trx_min_common_level(
                            path, gas_path
                        )
                        # 有共同gas激活祖先
                        if level < 999:
    ...
    # def get_score_for_adam(self, rank1_end, rank2_end, start_address, addresses_df, max_node):
    #     # 计算addresses_df中每个地址在nodes_data中的排名，地址可分为三类，需要输入三类地址的排名起始位置，第一类的评分为3*排名，第二类的评分为2*排名，第三类的评分为1*排名
    #     # 计算nodes_df
    #     nodes_df, _ = self.explore_illegal_routes(
    ...
    #             pass
    #         else:
    #             print(self.tensor_dict[address])
    #             scores = scores + 3 * self.tensor_dict[address]
    #     for address in rank2:
    #         # 如果在nodes_df中找不到该地址
    #         if address not in list(nodes_df["id"]):
    #             pass
    #         else:
    #             print(self.tensor_dict[address])
    #             scores = scores + 2 * self.tensor_dict[address]
    #     for address in rank3:
    #         # 如果在nodes_df中找不到该地址
    #         if address not in list(nodes_df["id"]):
    #             pass
    #         else:
    #             print(self.tensor_dict[address])
    #             scores = scores + 1 * self.tensor_dict[address]
    #     return scoresdependencies
...
if __name__ == "__main__":
    ...
    # -0.01到0.01的(1,19)向量
    W = np.random.uniform(-0.000001, 0.000001, size=(1, 19))
    gamma = 0
